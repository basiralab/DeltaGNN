...
def create_empty_file_result(filename):
    ...
    with open(filename, 'w') as f:
        f.write("Dataset,Model_Type,Epochs,Node_Num,Session_Memory,Train_Memory,Train_Time_Avg,Max_Val_Acc,"
                ...
                "Make_Unbalanced,Dense,Flow_Control,Linear\n")
...
def write_result(dataset, model_type, epochs, node_num, max_val_acc, max_val_f1, 
                 ...
                 remove_edges=None, topological_measure=None, make_unbalanced=None, dense=None, flow_control = None, linear = None):
    """
    ...
    with open(filename, 'a') as f:
        ...
        f.write(f"{flow_control if flow_control is not None else '-'},")
        f.write(f"{linear if linear is not None else '-'}\n")

def write_result_regression(dataset, model_type, epochs, num_features, min_mae_val, max_r2_val,
                             min_mse_val, min_rmse_val, min_mae_test, max_r2_test, 
                             min_mse_test, min_rmse_test, session_memory, train_memory,
                             train_time_avg, filename="result_regression.csv", 
                             hidden_dimension=None, max_communities=None, 
                             remove_edges=None, topological_measure=None, 
                             make_unbalanced=None, dense=None, flow_control = None, linear = None):
    """
    Appends regression results to a CSV file for a specific experiment.
    ...
    Args:
        ...
        min_mae_val (float): Minimum validation Mean Absolute Error achieved.
        max_r2_val (float): Maximum validation R-squared achieved.
        min_mse_val (float): Minimum validation Mean Squared Error achieved.
        min_rmse_val (float): Minimum validation Root Mean Squared Error achieved.
        min_mae_test (float): Minimum test Mean Absolute Error achieved.
        max_r2_test (float): Maximum test R-squared achieved.
        min_mse_test (float): Minimum test Mean Squared Error achieved.
        min_rmse_test (float): Minimum test Root Mean Squared Error achieved.
        session_memory (float): Peak memory usage during the session (in MB).
        train_memory (float): Memory usage during training (in MB).
        train_time_avg (float): Average training time per epoch (in seconds).
        filename (str): The name of the file to append results to. Default is "result_regression.csv".
        hidden_dimension (int or None): Dimension of hidden layers, if applicable.
        max_communities (int or None): Maximum number of communities, if applicable.
        remove_edges (int or None): Number of edges removed, if applicable.
        topological_measure (str or None): Topological measure used, if applicable.
    ...
    with open(filename, 'a') as f:
        ...
        f.write(f"{min_mae_val},")
        f.write(f"{max_r2_val},")
        f.write(f"{min_mse_val},")
        f.write(f"{min_rmse_val},")
        f.write(f"{min_mae_test},")
        f.write(f"{max_r2_test},")
        f.write(f"{min_mse_test},")
        f.write(f"{min_rmse_test},")
        f.write(f"{session_memory},")
        f.write(f"{train_memory},")
        f.write(f"{train_time_avg},")
        f.write(f"{hidden_dimension if hidden_dimension is not None else '-'},")
